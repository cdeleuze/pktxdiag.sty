...
\end{chrono}}
\hfill
\subfloat[X25]{\begin{chrono}[space other=8mm, space same=12mm, hide=main,show
  label2, hide label]
\figxix
\end{chrono}}
\hfill
...
\begin{hchrono}[hdlc,hide=main,show label2,hide label]
\figxix
\end{hchrono}
...
\begin{fchrono}[prop=1cm,width=6cm,A=émetteur,B=récepteur]{Contrôle de flot TCP}
  \newcommand{\buf}[3][0cm]{
    \begin{scope}[shift={(\chronowidth/2+1cm,\chronoTrecv{B}-3.75mm-#1)},x=7.5mm]
...
\buf{0}{0}
\node at (\chronowidth/2+2.5cm,-1mm) {tampon récepteur};
...
\begin{chrono}[width=5cm,prop=2mm]
  \ftp
\end{chrono}
\begin{chrono}[width=5cm,prop=2mm,hide=data,hide=tcp]
  \ftp
\end{chrono}
\begin{chrono}[prop=2mm,hide=main,hide=tcp]
