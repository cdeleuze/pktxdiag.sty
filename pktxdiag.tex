...
If you install the package manually, simply ensure the file
\texttt{pktxdiag.sty} is in \LaTeX's load path.  A quick and easy way
to give it a try is to copy it in the directory from which you run the
...
Currently four timers may be run simultaneously on each side.
...
\begin{pktxdiag}[debug,timers/tight,reversed]
  \fullduplex
  \AB[timed={4cm}{},lost,cross for lost,label style=midway]{DATA 0}
...
colors, line width, arrow tip shape etc.

\begin{codeexample}[width=5cm]
  \begin{pktxdiag}
    \AB[thin]{DATA}
    \BA[dashed,blue,-Latex]{ACK}
    \AB[ultra thick,yellow,text=red,
...
can be redefined to change the style of a given entity.  This can be usefull in diagrams with multiple entities, see section \ref{sec:multipleentities}.
...
features include: late frames, laters, states and actions.
...
%TODO: also need -alpha off
% convert={density=100,command=\unexpanded{{\convertexe\space -density \density\space \infile\space -alpha off \outfile}}}
%do convert manually

%convert -alpha off -density 100 file.pdf file.png
